<li><a href="{{ url('AdminAnalystOD/list_matrikindikator') }}"><i class="fa fa-circle-o"></i>Matrikindikator</a></li>
<li><a href="{{ url('AdminAnalystOD/list_bahankerja') }}"><i class="fa fa-circle-o"></i>Bahan Kerja</a></li>
<li><a href="{{ url('AdminAnalystOD/list_katakerja') }}"><i class="fa fa-circle-o"></i>Kata Kerja (Main Responsibility)</a></li>
<li><a href="{{ url('AdminAnalystOD/list_pengalamankerja') }}"><i class="fa fa-circle-o"></i>pengalaman kerja</a></li>
<li><a href="{{ url('AdminAnalystOD/list_pendidikan') }}"><i class="fa fa-circle-o"></i>Pendidikan</a></li>
<li><a href="{{ url('AdminAnalystOD/list_alatkerja') }}"><i class="fa fa-circle-o"></i>Alat Kerja</a></li>
<li><a href="{{ url('AdminAnalystOD/list_lingkungankerja') }}"><i class="fa fa-circle-o"></i>Lingkungan Kerja</a></li>
<li><a href="{{ url('AdminAnalystOD/list_persyaratan') }}"><i class="fa fa-circle-o"></i>persyaratan_fisik</a></li>